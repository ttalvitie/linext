...
sizes = [8, 10, 12, 16, 20, 24, 32, 40, 50, 64, 80, 100, 128, 160, 202, 256, 322, 406, 512, 644, 812, 1024]
...
def generate_random_sparses():
	...
	for m in range(5):
		for k in [3,5]:
			for n in sizes:
				dag.save("avgdeg-%i_%04i_%i" % (k, n, m), dag.random_average_degree(n, k))
	for m in range(5):
		for k in [7]:
			for n in sizes:
				dag.save("avgdeg-%i_%04i_%i" % (k, n, m), dag.random_average_degree(n, k))
...
def generate_random_bipartites():
	...
	for m in range(5):
		for p in [0.2, 0.5]:
			for n in half_sizes:
				dag.save("bipartite-%s_%04i_%i" % (str(p), 2 * n, m), dag.random_bipartite(n, p))
				dag.save("bipartite-%s_%04i_%i" % (str(p), 2 * n, m), dag.random_bipartite(n, p))
	...
	for network in network_names:
		...
		for i in sizes:
			for m in range(5):
				...
				if dg is None:
					continue
				dag.save("bayesiannetwork-%s_%04i_%i" % (network, i, m), dg)
