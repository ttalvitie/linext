...
ll tpaFinalWalkCount(double epsilon, double delta, double prelimEstimate) {
    ...
    return safePositiveCeil(
        2.0 * (prelimEstimate + sqrt(prelimEstimate) + 2.0 + epsilon)
        * log(4.0 / delta)
        / (epsilon * epsilon)
    );
}
...
template <int W>
void method_relaxtpa_loose2(const Poset<W>& poset, double epsilon, double delta) {
    runBasicRelaxationTPAWithAlternativeParTPA<W, loose2_::ParallelTPAContext>(poset, epsilon, delta);
}

#define FOR_EACH_POSET_W_ELEM(W) \
    template void method_relaxtpa_loose2<W>(const Poset<W>&, double, double);
FOR_EACH_POSET_W
